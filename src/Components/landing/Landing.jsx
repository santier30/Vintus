...
const Landing = ()=>{
    ...
    const fetchLand =  useCallback(async()=>{
        try {
            const response1 = await fetch(`${process.env.REACT_APP_API_URL}/Vintus/Products/Know`)
            const fKnow = await response1.json()
            const response2 = await fetch(`${process.env.REACT_APP_API_URL}/Vintus/Products/Exclusive`)
